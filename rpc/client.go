...
type Client struct {
	...
	reconnectFunc        reconnectFunc // Called when connection is lost

	batchItemLimit       int // Maximum number of items in a batch request.
	batchResponseMaxSize int // Maximum response size.

	writeConn jsonWriter // Used for writing to the connection.

	close       chan struct{}    // Channel to signal closure.
	closing     chan struct{}    // Closed when client is quitting.
	didClose    chan struct{}    // Closed when client quits.
	reconnected  chan ServerCodec // Where write/reconnect sends the new connection.
	readOp      chan readOp      // Channel for read messages.
	readErr     chan error       // Channel for read errors.
...
// requestOp represents a pending request. This is used for both batch and non-batch requests.
...
// DialOptions creates a new RPC client for the given URL with options.
...
// RegisterName creates a service for the given receiver type under the given name. When no methods on the given receiver match the criteria to be either an RPC method or a subscription an error is returned. Otherwise a new service is created and added to the service collection this client provides to the server.
...
// SupportedModules calls the rpc_modules method to retrieve available APIs on the server.
...
// Close closes the client and aborts any in-flight requests.
func (c *Client) Close() {
	if c.isHTTP { 
        return 
    }
...
// This method only works for clients using HTTP; it doesn't affect other transports.
...
// Call performs a JSON-RPC call with given arguments and unmarshals into result if no error occurred.
// The result must be a pointer so that package json can unmarshal into it. You can also pass nil if you want to ignore it.
...
// If the context is canceled before successfully returning CallContext returns immediately.
// The result must be a pointer so that package json can unmarshal into it. You can also pass nil if you want to ignore it.
...
// In contrast to CallBatchCall only returns I/O errors. Any specific request errors are reported through BatchElem's Error field.
// Note that batch calls may not be executed atomically on server side.
...
// In contrast to CallContext BatchCallContext only returns errors that have occurred while sending requests.
// Any specific request errors are reported through BatchElem's Error field.
// Note that batch calls may not be executed atomically on server side.
...
func (c *Client) ShhSubscribe(ctx context.Context ,channel interface{},args ...interface{}) (*ClientSubscription,error ) { return c.Subscribe(ctx ,"shh",channel,args...) }
